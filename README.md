...
2. pyroomacoustics で音源数とマイク数が等しい BSS 条件を作る
...
8. wav, スペクトログラム, config, metrics を `results/` に保存する

## 前提

この README のコマンドは，すべて `bss-handson/` ディレクトリ内で実行する．
`uv` は実行するディレクトリにある `pyproject.toml` を見てプロジェクトを判断するため，別ディレクトリで実行すると依存関係や CLI entry point の解決結果が変わる．

```bash
cd bss-handson
```

Python プロジェクトは `uv init --lib` 相当の構成で作られている．
`src/bss_handson/` 以下が import されるパッケージ本体であり，`pyproject.toml` の `[project.scripts]` により `bss-handson` コマンドを登録している．
...
たとえば，STFT，シミュレーション，AuxIVA，評価指標，スペクトログラム保存のような単機能の処理である．
...
`plot.style` は Matplotlib のスタイルファイルのリストであり，共通設定と媒体別設定を順に読み込む．
...
このプロジェクトでは，`src/bss_handson/` に単機能の処理と共通 pipeline を置き，`scripts/run_bss.py` は標準実験の入口と保存処理だけを持つ．
...
プレゼン向けのスタイルを使う場合は，`styles/common.mplstyle` と `styles/slide.mplstyle` を読み込む．
...
`plt.style.use()` はスタイルファイルのリストを受け取れるため，共通設定と媒体別設定を次のように重ねて使う．
...
| `pipeline.py`   | 標準実験と random evaluation で共有する実行手順    |
| `plot.py`       | スペクトログラムの保存                             |
| `plot_style.py` | Matplotlib の軸スタイルの適用                       |
| `cli.py`        | `scripts/run_bss.py` を呼ぶ最小限の entry point    |

`src/bss_handson/` は，1 つの意味のある入力に対して 1 つの結果を返す小さな処理を置く場所である．
この分け方は，McIlroy による UNIX philosophy の「一つのことをうまく行う program を書き，互いに組み合わせる」という考え方に合わせている（[The Creation of the UNIX* Operating System: Creating a programming philosophy from pipes and a tool box](https://www.nokia.com/bell-labs/unix-history/philosophy.html)）．
関数を小さく保つと，別の実験で再利用しやすくなるだけでなく，発展的に単体テストを書くときにも形状，値の範囲，保存形式を個別に検証しやすい．
`scripts/` は，それらの処理を組み合わせて実験実行，条件 sweep，集計，可視化を行う場所である．
標準実験と random evaluation で共有する処理は `src/bss_handson/pipeline.py` に置き，`scripts/run_bss.py` は出力先の作成と結果保存を担当する．

| スクリプト                             | 役割                                                |
| ---------------------------------- | --------------------------------------------------- |
| `run_bss.py`                       | 標準 BSS 実験の入口と結果保存                       |
...
このプロジェクトでは次の設定により CLI command を登録している．
