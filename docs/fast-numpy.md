...
正方行列同士では $O(N ^ 3)$ になる．
BLAS は cache，SIMD，複数 thread を利用できるが，古典的なアルゴリズムの漸近計算量は変わらない．
...
NumPy はこの計算に，**線形代数パッケージ** (Linear Algebra PACKage, LAPACK) の `gesv` routine を利用する[^lapack-gesv]．
...
残差と実行時間は，入力，dtype，NumPy が利用する線形代数ライブラリによって変わる．
...
- 実行方法については，反復回数の大きな軸を ufunc，集約演算，`@`，batched `solve()` へ移し，dtype，メモリ配置，copy も確認する．
