...
absorption, max_order = pra.inverse_sabine(room_config["rt60"], room_config["size"])
room = pra.ShoeBox(
    room_config["size"],
    fs=fs,
...
win = signal.get_window(stft_config["window"], stft_config["win_length"])
stft = signal.ShortTimeFFT(win, hop=stft_config["hop"], fs=fs)
...
for prefix, signals_to_plot, title in [
    ...
    for index, spectrum in enumerate(plot_spectra):
        ...
        image = ax.imshow(
            ...
            vmin=plot_config["vmin_db"],
            vmax=plot_config["vmax_db"],
        )
